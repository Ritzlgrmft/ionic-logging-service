...
describe("LoggingService", () => {
	...
	describe("with ConfigurationService", () => {
	...
	describe("without ConfigurationService", () => {
		...
		describe("configure(configuration: LoggingServiceConfiguration): void", () => {
			...
			it("uses default configuration", () => {
				...
				const internalLogger = new Logger().getInternalLogger();
				expect(internalLogger.getEffectiveLevel()).toBe(log4javascript.Level.WARN);

				const appenderCount = internalLogger.getEffectiveAppenders().length;
				expect(appenderCount).toBe(2);
			});
		});
	});
});
